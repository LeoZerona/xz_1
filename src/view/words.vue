...
const filteredWords = computed(() => {
  ...
  return commonWords.filter(
    (word) =>
      word.jt.includes(searchKey.value) || word.ft.includes(searchKey.value)
  );
});
...
.words-container {
  min-height: calc(
    100vh - 100px
  ); // 减去顶部导航栏高度（60px），使用的是标注盒模型，所以还要删除padding的40px
  padding: 20px;
...
@media (max-width: 768px) {
  .words-container {
    min-height: calc(100vh - 50px); // 减去顶部导航栏高度（50px）
    padding: 15px;
...
@media (max-width: 480px) {
  .words-container {
    min-height: calc(100vh - 50px); // 减去顶部导航栏高度（50px）
    padding: 10px;
    padding-top: 58px; // 超小屏幕 navigation 高度（50px）+ 间距（8px）
