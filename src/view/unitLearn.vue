...
const { learnConfig } = storeToRefs(learnInfoHome) as { learnConfig: Ref<LearnConfigType> }
...
function initData() {
  unitInfoClone.value = JSON.parse(JSON.stringify(unitInfo.value))
  ...
  switch (learnConfig.value.model) {
    case 'select':
      for (let i = 0; i < unitInfoClone.value.characters.length; i++) {
        ...
        const option = getUniqueRandomCharacters(unitInfoClone.value.characters, 3)
        option.splice(Math.floor(Math.random() * (option.length + 1)), 0, unitInfoClone.value.characters[i])
        unitInfoClone.value.model.options.push(option);
      }
      break;
    case 'write':
      unitInfoClone.value.model.types = new Array(unitInfoClone.value.characters.length).fill('write')
      break;
    case 'mix':
      for (let i = 0; i < unitInfoClone.value.characters.length; i++) {
...
function getUniqueRandomCharacters(sourceArray:Array<string>, count:number) {
  ...
  while (uniqueCharacters.size < count) { // 循环直到集合中的元素数量达到需要的数量
